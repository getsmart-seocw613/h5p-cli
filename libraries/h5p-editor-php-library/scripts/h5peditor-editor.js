...
ns.Editor = function (library, defaultParams, replace, iframeLoaded) {
  ...
  var load = function () {
    ...
    ///
    console.log(this.contentWindow.H5PEditor.getAjaxUrl);
    // Load libraries data
    $.ajax({
...
ns.Editor.prototype.getLibrary = function () {
  console.log(this.selector.getCurrentLibrary);
...
ns.Editor.prototype.getContent = function (submit, error) {
  ...
  const content = {
    /// 제목이 있으면 제목 반환, 없으면 제목 input focus
